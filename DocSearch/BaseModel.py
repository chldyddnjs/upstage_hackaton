...
class bge:
    ...
    def top_k(self,result,k):
        sorted_result = sorted(result.items,lambda x:x[1])
        return sorted_result[:k]

    def search(self,data_path:str,output_path:str,threshold:float):
        #'legal.jsonl'

        dataset = load_dataset('json',data_files=data_path,split="train[:10]",num_proc=os.cpu_count()-2)
        ...
        sentence_pairs = [[i,j] for i in questions[0] for j in documents]
